...
public class CassandraAppender extends AppenderBase<ILoggingEvent> {
	...
	private synchronized void initClient() {
		...
		try {
			Cluster.Builder builder = Cluster.builder().addContactPoints(hosts.split(",\\s*")).withPort(port)
	...
	private void setupSchema() throws IOException {
		...
		String ksQuery = String.format("CREATE KEYSPACE IF NOT EXISTS \"%s\" WITH REPLICATION = %s;", keyspaceName,
				replication);
		...
		String cfQuery = String.format("CREATE TABLE IF NOT EXISTS \"%s\".\"%s\" (%s UUID PRIMARY KEY, "
				...
				+ "%s text, %s text, %s bigint, %s text, %s text, %s text, %s text," + "%s text);", keyspaceName,
				columnFamily, ID, APP_NAME, APP_START_TIME, CLASS_NAME, FILE_NAME, HOST_IP, HOST_NAME, LEVEL,
				LINE_NUMBER, METHOD_NAME, TIMESTAMP, LOGGER_NAME, MESSAGE, NDC, THREAD_NAME, THROWABLE_STR);
	...
	private void setupStatement() {
		...
		String insertQuery = String.format("INSERT INTO \"%s\".\"%s\" "
				...
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?); ", keyspaceName, columnFamily, ID,
				APP_NAME, HOST_IP, HOST_NAME, LOGGER_NAME, LEVEL, CLASS_NAME, FILE_NAME, LINE_NUMBER, METHOD_NAME,
				MESSAGE, NDC, APP_START_TIME, THREAD_NAME, THROWABLE_STR, TIMESTAMP);
	 ...
	 */
	private void createAndExecuteQuery(ILoggingEvent event) {
		...
		if (callerData != null) {
			bound.setString(6, callerData[0].getClassName());
			bound.setString(7, callerData[0].getFileName());
			bound.setString(8, callerData[0].getLineNumber() + "");
			bound.setString(9, callerData[0].getMethodName());
		}

		bound.setString(10, event.getMessage());
		bound.setString(11, event.getMdc().toString());
		bound.setLong(12, event.getTimeStamp());
		bound.setString(13, event.getThreadName());

		if (event.getThrowableProxy() != null){
			IThrowableProxy[] throwableStrs = event.getThrowableProxy().getSuppressed();
	...
	private SSLOptions getSslOptions() throws Exception {
		...
		ctx.init(kmf == null ? null : kmf.getKeyManagers(), tmf == null ? null : tmf.getTrustManagers(),
				new SecureRandom());
